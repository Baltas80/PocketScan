...
object AiLibraryAssistant {
    ...
    private suspend fun tryCloud(question: String, context: String): Result<String> = runCatching {
        ...
        val prompt = """
            ...
            $context
        """.trimIndent()
        model.generateContent(prompt).text?.trim()?.takeIf { it.isNotBlank() } ?: error("La IA no devolvió respuesta")
    }
    ...
    private fun localAnswer(question: String, context: String): String {
        ...
        return if (matches.isEmpty()) "No encuentro información suficiente en la biblioteca para responder a esa pregunta."
        else "No hay conexión con la IA en la nube. Estos son los documentos locales relacionados:\n\n" + matches.joinToString("\n\n---\n\n")
    }

    private fun normalize(value: String): String = java.text.Normalizer.normalize(value.lowercase(), java.text.Normalizer.Form.NFD).replace("\\p{InCombiningDiacriticalMarks}+".toRegex(), "")
}
