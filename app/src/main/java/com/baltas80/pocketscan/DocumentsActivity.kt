...
class DocumentsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val categories = listOf(getString(R.string.all_categories)) + DocumentOrganizer.categories
    ...
    private fun applyFilters() {
        val query = searchInput.text?.toString().orEmpty().trim().lowercase(); val selected = categorySpinner.selectedItem?.toString() ?: getString(R.string.all_categories); val snapshot = allDocuments.toList()
        lifecycleScope.launch {
            val filtered = withContext(Dispatchers.IO) {
                snapshot.filter { file ->
                    val category = DocumentOrganizer.categoryForFile(file, scansDir)
                    if (selected != getString(R.string.all_categories) && category != selected) return@filter false
    ...
    private fun shareDocument(file: File) {
        ...
        startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply { type = "application/pdf"; putExtra(Intent.EXTRA_STREAM, uri); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }, getString(R.string.share_document)))
    }
    ...
    private fun prepareImportedImages(uris: List<Uri>): ImportFiles? {
        scansDir.mkdirs(); val stamp = System.currentTimeMillis(); val pdf = File(scansDir, "document_import_$stamp.pdf"); val text = File(scansDir, "document_import_$stamp.txt"); val tempFiles = mutableListOf<File>()
        return try { uris.forEachIndexed { index, uri -> val temp = File(scansDir, "import_${stamp}_$index.jpg"); contentResolver.openInputStream(uri)?.use { input -> FileOutputStream(temp).use { output -> input.copyTo(output) } } ?: error("No se pudo leer una imagen"); tempFiles.add(temp) }; ImportFiles(tempFiles, pdf, text) } catch (_: Exception) { tempFiles.forEach { it.delete() }; pdf.delete(); text.delete(); null }
    }
    ...
    private fun createPdfFromImages(files: List<File>, pdfFile: File) {
        val document = PdfDocument(); try { files.forEachIndexed { index, file -> val bitmap = BitmapFactory.decodeFile(file.absolutePath) ?: error("Imagen no válida"); val page = document.startPage(PdfDocument.PageInfo.Builder(595, 842, index + 1).create()); val margin = 24f; val scale = minOf((595f - margin * 2) / bitmap.width, (842f - margin * 2) / bitmap.height); val width = bitmap.width * scale; val height = bitmap.height * scale; val left = (595f - width) / 2f; val top = (842f - height) / 2f; page.canvas.drawBitmap(bitmap, null, RectF(left, top, left + width, top + height), null); document.finishPage(page); bitmap.recycle() }; FileOutputStream(pdfFile).use { document.writeTo(it) } } finally { document.close() }
    }

    private fun runOcr(files: List<File>, textFile: File, onComplete: () -> Unit) {
        val recognizer = com.google.mlkit.vision.text.TextRecognition.getClient(com.google.mlkit.vision.text.latin.TextRecognizerOptions.DEFAULT_OPTIONS); val all = StringBuilder()
        fun complete() { lifecycleScope.launch(Dispatchers.IO) { runCatching { textFile.writeText(all.toString(), Charsets.UTF_8) }; withContext(Dispatchers.Main) { recognizer.close(); onComplete() } } }
        fun next(index: Int) { if (index >= files.size) { complete(); return }; try { val image = com.google.mlkit.vision.common.InputImage.fromFilePath(this, Uri.fromFile(files[index])); recognizer.process(image).addOnSuccessListener { result -> if (result.text.isNotBlank()) { if (all.isNotEmpty()) all.append("\n\n"); all.append(result.text) }; next(index + 1) }.addOnFailureListener { next(index + 1) } } catch (_: Exception) { next(index + 1) } }
        next(0)
    }
    ...
    private fun renameDocument(file: File) {
        ...
        AlertDialog.Builder(this).setTitle(R.string.rename_document).setView(input).setNegativeButton(android.R.string.cancel, null).setPositiveButton(R.string.rename) { _, _ ->
            val newName = sanitizeFileName(input.text.toString().trim()); if (newName.isEmpty()) return@setPositiveButton; val target = File(file.parentFile, "$newName.pdf")
            if (target.exists()) { Toast.makeText(this, R.string.file_already_exists, Toast.LENGTH_SHORT).show(); return@setPositiveButton }
            lifecycleScope.launch { val success = withContext(Dispatchers.IO) { if (!file.renameTo(target)) false else { File(file.parentFile, file.nameWithoutExtension + ".txt").renameTo(File(file.parentFile, "$newName.txt")); AiMetadataStore.sidecarFor(file).renameTo(File(file.parentFile, "$newName.ai.json")); true } }; if (success) loadDocuments() else Toast.makeText(this@DocumentsActivity, R.string.rename_failed, Toast.LENGTH_SHORT).show() }
        }.show()
    }
    ...
    private fun deleteDocument(file: File) {
        AlertDialog.Builder(this).setTitle(R.string.delete_document).setMessage(file.name).setNegativeButton(android.R.string.cancel, null).setPositiveButton(R.string.delete) { _, _ ->
            lifecycleScope.launch { val deleted = withContext(Dispatchers.IO) { val result = file.delete(); File(file.parentFile, file.nameWithoutExtension + ".txt").delete(); AiMetadataStore.delete(file); result }; if (deleted) loadDocuments() else Toast.makeText(this@DocumentsActivity, R.string.delete_failed, Toast.LENGTH_SHORT).show() }
        }.show()
    }
}
