...
object AiDocumentAnalyzer {
    ...
    suspend fun analyze(file: File, ocrText: String = ""): Result<Analysis> = withContext(Dispatchers.IO) { runCatching { require(file.isFile) { "Document not found" }; val text=ocrText.trim(); tryCloudAnalysis(file,text).getOrElse { localAnalysis(file,text) } } }
    private suspend fun tryCloudAnalysis(file: File, ocrText: String): Result<Analysis> = runCatching {
        ...
        val model: GenerativeModel=Firebase.ai(backend=GenerativeBackend.googleAI(),useLimitedUseAppCheckTokens=true).generativeModel(modelName=AiModelConfig.modelName(),generationConfig=generationConfig { responseMimeType="application/json"; responseSchema=documentSchema })
        val auxiliaryOcr=ocrText.take(12000); val prompt=content { inlineData(file.readBytes(),"application/pdf"); text("""You are PocketScan's document intelligence engine. Analyze the complete PDF, including layout, tables and visible values. OCR text below is only auxiliary context.
...
$auxiliaryOcr""".trimIndent()) }
    ...
    private fun localAnalysis(file: File,text:String): Analysis {
        ...
        monetaryField(text,Regex("(?i)^(?:subtotal|base imponible|base)\\s*[:=]?\\s*(?:(€|EUR|USD|\\$|GBP|£)\\s*)?([0-9][0-9.,]*)(?:\\s*(€|EUR|USD|\\$|GBP|£))?\\s*$"))?.let{m->fields["subtotal"]=m.value;if(!fields.containsKey("moneda"))m.currency?.let{fields["moneda"]=normalizeCurrency(it)}}
        firstMatch(text,Regex("(?im)^\\s*(?:periodo|per[ií]odo|ejercicio|campaign|campa[nñ]a)\\s*[:.-]?\\s*([^\\r\\n]{2,60}?)\\s*$"))?.let{fields["periodo"]=cleanField(it)}; firstMatch(text,Regex("(?im)^\\s*(?:tel[eé]fono|tel\\.?|phone|telephone)\\s*[:.-]?\\s*([+0-9][0-9 ()-]{6,24})\\s*$"))?.let{fields["telefono"]=cleanField(it)}; firstMatch(text,Regex("(?im)^\\s*(?:direcci[oó]n|domicilio|address)\\s*[:.-]?\\s*([^\\r\\n]{4,100}?)\\s*$"))?.let{fields["direccion"]=cleanField(it)}; firstMatch(text,Regex("(?im)^\\s*(?:concepto|asunto|motivo|description|descripci[oó]n)\\s*[:.-]?\\s*([^\\r\\n]{3,120}?)\\s*$"))?.let{fields["concepto"]=cleanField(it)}; firstMatch(text,Regex("(?im)^\\s*(?:proveedor|emisor|empresa|entidad|supplier|vendor|issuer|company)\\s*[:.-]?\\s*([^\\r\\n]{3,100}?)\\s*$"))?.let{fields["proveedor"]=cleanField(it)}; firstMatch(text,Regex("(?im)^\\s*(?:cliente|destinatario|beneficiario|titular|customer|client|recipient|beneficiary|account holder)\\s*[:.-]?\\s*([^\\r\\n]{3,100}?)\\s*$"))?.let{fields["cliente"]=cleanField(it)}
        val summary=if(text.isBlank())"No OCR disponible para un análisis local más preciso." else text.lineSequence().map{it.trim()}.filter{it.isNotBlank()&&!isOcrMarkerLine(it)}.joinToString(" ").replace(Regex("\\s+")," ").trim().take(700); return Analysis(category,title,summary,fields)
    }
    ...
    private fun normalizeCategory(value:String)=when(value.trim().uppercase(Locale.ROOT)){"FACTURAS"->DocumentOrganizer.FACTURAS;"PRESUPUESTOS"->DocumentOrganizer.PRESUPUESTOS;"CONTRATOS"->DocumentOrganizer.CONTRATOS;"RECIBOS"->DocumentOrganizer.RECIBOS;"TICKETS"->DocumentOrganizer.TICKETS;"NOMINAS"->DocumentOrganizer.NOMINAS;"CERTIFICADOS"->DocumentOrganizer.CERTIFICADOS;"INFORMES"->DocumentOrganizer.INFORMES;"CITAS"->DocumentOrganizer.CITAS;else->DocumentOrganizer.GENERAL}
    private fun languageName()=when(Locale.getDefault().language.lowercase(Locale.ROOT)){"es"->"Spanish (Spain)";"en"->"English";"fr"->"French";"de"->"German";"it"->"Italian";"pt"->"Portuguese";"ca"->"Catalan";else->Locale.getDefault().displayLanguage}
}
