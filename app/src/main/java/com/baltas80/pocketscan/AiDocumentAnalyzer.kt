...
object AiDocumentAnalyzer {
    ...
    suspend fun analyze(file: File, ocrText: String = ""): Result<Analysis> = withContext(Dispatchers.IO) {
        runCatching { require(file.isFile) { "Document not found" }; val text = ocrText.trim(); tryCloudAnalysis(text).getOrElse { localAnalysis(file, text) } }
    }
    ...
    private suspend fun tryCloudAnalysis(ocrText: String): Result<Analysis> = runCatching {
        ...
        val prompt = """
            You are PocketScan's document intelligence engine. Analyze the OCR text and return ONLY valid JSON.
            Never invent data. If a value is absent, use an empty string.
        ...
        if (fieldsJson != null) fieldsJson.keys().forEach { key -> fieldsJson.optString(key).trim().takeIf { it.isNotEmpty() }?.let { fields[key] = it } }
    ...
    private fun localAnalysis(file: File, text: String): Analysis {
        ...
        val usefulLine = text.lines().map { it.trim() }.firstOrNull { it.length >= 4 && !it.equals(type, true) && !it.matches(Regex("[0-9 ./,:-]+")) }
        ...
        return Analysis(category, title, if (text.isBlank()) "No OCR available for a more precise local analysis." else "Local classification based on OCR text.", fields, "local")
    }

    private fun firstMatch(text: String, regex: Regex): String? = regex.find(text)?.groupValues?.getOrNull(1)?.trim()?.takeIf { it.isNotEmpty() }
    private fun normalizeCategory(value: String): String = value.trim().uppercase().let { if (it in DocumentOrganizer.categories) it else DocumentOrganizer.GENERAL }
    private fun sanitizeTitle(value: String): String = value.replace(Regex("\\s+"), " ").replace(Regex("[\\r\\n]+"), " ").trim()
    private fun extractJson(raw: String): String { val cleaned = raw.substringAfter("```json", raw).substringBeforeLast("```").trim(); val start = cleaned.indexOf('{'); val end = cleaned.lastIndexOf('}'); require(start >= 0 && end > start) { "Invalid AI response" }; return cleaned.substring(start, end + 1) }
    private fun languageName(): String = when (Locale.getDefault().language.lowercase(Locale.ROOT)) { "es" -> "Spanish"; "en" -> "English"; "fr" -> "French"; "de" -> "German"; "it" -> "Italian"; "pt" -> "Portuguese"; "ca" -> "Catalan"; "ar" -> "Arabic"; "nl" -> "Dutch"; "pl" -> "Polish"; "tr" -> "Turkish"; "ja" -> "Japanese"; "ko" -> "Korean"; "zh" -> "Chinese"; "ru" -> "Russian"; else -> "English" }
}
