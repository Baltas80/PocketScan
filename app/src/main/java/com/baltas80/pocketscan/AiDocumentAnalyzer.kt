...
object AiDocumentAnalyzer {
    ...
    private suspend fun tryCloudAnalysis(file: File, ocrText: String): Result<Analysis> = runCatching {
        ...
        val model: GenerativeModel = Firebase.ai(backend = GenerativeBackend.googleAI(), useLimitedUseAppCheckTokens = true)
            .generativeModel(modelName = AiModelConfig.modelName(), generationConfig = generationConfig {
        ...
        val prompt = content {
            ...
            text("""You are PocketScan's document intelligence engine. Analyze the complete PDF, including layout, tables and visible values. OCR text below is only auxiliary context.
...
$auxiliaryOcr""".trimIndent())
        ...
        val json = JSONObject(model.generateContent(prompt).text ?: error("AI returned no content"))
        ...
        verifyCriticalMonetaryFields(file, base)
    }

    /**
     * Gemini is an interpretation layer, not the source of truth for a monetary total.
     * For financial document categories, replace a model-provided total only with an
     * independently spatially verified OCR candidate. If no such candidate exists,
     * remove the total rather than persist an unverified amount.
     */
    private fun verifyCriticalMonetaryFields(file: File, analysis: Analysis): Analysis {
    ...
    private fun localAnalysis(file: File, text: String): Analysis {
        ...
        val type = when(category){DocumentOrganizer.FACTURAS->"Factura";DocumentOrganizer.PRESUPUESTOS->"Presupuesto";DocumentOrganizer.CONTRATOS->"Contrato";DocumentOrganizer.RECIBOS->"Recibo";DocumentOrganizer.TICKETS->"Ticket";DocumentOrganizer.NOMINAS->"Nomina";DocumentOrganizer.CERTIFICADOS->"Certificado";DocumentOrganizer.INFORMES->"Informe";DocumentOrganizer.CITAS->"Cita";else->"Documento"}
        val usefulLine=text.lineSequence().map{it.trim()}.firstOrNull{it.length>=4&&it.any(Char::isLetter)&&it.count(Char::isDigit)<it.length/2&&!it.equals(type,true)&&!isOcrMarkerLine(it)&&!isGenericHeaderLine(it)}
        val title=usefulLine?.replace(Regex("\\s+")," ")?.take(70)?.ifBlank{"$type - ${file.nameWithoutExtension}"} ?: "$type - ${file.nameWithoutExtension}"
        val fields=linkedMapOf<String,String>()
        monetaryField(text,Regex("(?i)^(?:total|importe total|total\\s+amount|montant total|gesamtbetrag|totale)\\s*[:=]?\\s*(?:(€|EUR|USD|\\$|GBP|£)\\s*)?([0-9][0-9.,]*)(?:\\s*(€|EUR|USD|\\$|GBP|£))?\\s*$"))?.let{m->fields["total"]=m.value;m.currency?.let{fields["moneda"]=normalizeCurrency(it)}}
    ...
    private fun isOcrMarkerLine(line:String):Boolean { val normalized=Normalizer.normalize(line,Normalizer.Form.NFD).replace(Regex("\\p{M}+"),"").lowercase(Locale.ROOT).trim(); return normalized.matches(Regex("^[=-]+\\s*pagina\\s+\\d+\\s*[=-]+$")) }
