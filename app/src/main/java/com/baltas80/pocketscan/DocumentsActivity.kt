...
class DocumentsActivity : AppCompatActivity() {
    ...
    private fun createPdfFromImages(files: List<File>, pdfFile: File) { val document = PdfDocument(); try { files.forEachIndexed { index, file -> val bitmap = BitmapFactory.decodeFile(file.absolutePath) ?: error("Invalid image"); val page = document.startPage(PdfDocument.PageInfo.Builder(595, 842, index + 1).create()); val margin = 24f; val scale = minOf((595f - margin * 2) / bitmap.width, (842f - margin * 2) / bitmap.height); val width = bitmap.width * scale; val height = bitmap.height * scale; val left = (595f - width) / 2f; val top = (842f - height) / 2f; page.canvas.drawBitmap(bitmap, null, RectF(left, top, left + width, top + height), null); document.finishPage(page); bitmap.recycle() }; FileOutputStream(pdfFile).use { document.writeTo(it) } } finally { document.close() } }
    private fun runOcr(files: List<File>, textFile: File, onComplete: () -> Unit) { val recognizer = com.google.mlkit.vision.text.TextRecognition.getClient(com.google.mlkit.vision.text.latin.TextRecognizerOptions.DEFAULT_OPTIONS); val all = StringBuilder(); fun complete() { lifecycleScope.launch(Dispatchers.IO) { runCatching { textFile.writeText(all.toString(), Charsets.UTF_8) }; withContext(Dispatchers.Main) { recognizer.close(); onComplete() } } }; fun next(index: Int) { if (index >= files.size) { complete(); return }; try { val image = com.google.mlkit.vision.common.InputImage.fromFilePath(this, Uri.fromFile(files[index])); recognizer.process(image).addOnSuccessListener { result -> if (result.text.isNotBlank()) { if (all.isNotEmpty()) all.append("\n\n"); all.append(result.text) }; next(index + 1) }.addOnFailureListener { next(index + 1) } } catch (_: Exception) { next(index + 1) } }; next(0) }
